...
static void pollSerialForDemo() {
  ...
  if (cmd.length() == 0) {
      DBG_PRINTLN("\nUse 'rpm', 'pattern', 'start', 'stop', or 'custom'. Type 'custom' for more info.");
      return;
  }
  ...
  if (cmd.startsWith("custom")) {
    ...
    DBG_PRINTLN("[CMD] Unknown command.");
  }
}
...
void setup() {
  ...
  DBG_PRINTLN("  custom              - Get help on the custom pattern builder");
  DBG_PRINTLN("  start               - Start or resume signal generation");
  DBG_PRINTLN("  stop                - Stop signal generation");
  DBG_PRINTLN("--------------------------------------------------");
