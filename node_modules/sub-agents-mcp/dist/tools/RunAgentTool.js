...
const SessionHistoryFormatter_1 = require("../session/SessionHistoryFormatter");
...
class RunAgentTool {
    constructor(agentExecutor, agentManager, sessionManager) {
        ...
        this.inputSchema = {
            ...
            properties: {
                ...
                prompt: {
                    type: 'string',
                    description: "User's direct request content. Agent context is separately provided via agent parameter.",
                },
                cwd: {
                    type: 'string',
                    description: 'Working directory path for agent execution context. Must be an absolute path to a valid directory.',
                },
                extra_args: {
                    type: 'array',
                    items: { type: 'string' },
    ...
    async execute(params) {
        ...
        try {
            ...
            const sessionId = validatedParams.session_id || (this.sessionManager ? (0, node_crypto_1.randomUUID)() : undefined);
                ...
                if (sessionId && this.sessionManager) {
                    try {
                        ...
                        if (sessionData && sessionData.history.length > 0) {
                            ...
                            const historyMarkdown = (0, SessionHistoryFormatter_1.formatSessionHistory)(sessionData);
    ...
    validateParams(params) {
        ...
        if (p['session_id'] !== undefined) {
            ...
            if (!/^[a-zA-Z0-9_-]+$/.test(sessionId)) {
                throw new Error('Session ID contains invalid characters (only alphanumeric, underscore, and dash allowed)');
            }
        }
        return {
            agent: agentName,
     ...
     */
    extractAgentContent(resultJson, isError, stdout, stderr) {
     ...
     */
    isAgentError(resultJson, exitCode) {
     ...
     */
    formatExecutionResponse(result, agentName, requestId, sessionId) {
        ...
        const contentText = this.extractAgentContent(result.resultJson, isError, result.stdout, result.stderr);
